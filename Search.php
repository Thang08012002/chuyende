      ...
      <div class="container-fluid">
         ...
         <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
               ...
               <li class="nav-item">
                  <a class="nav-link active" href="./cart.php">card</a>
               </li>
               <li class="nav-item">
         ...
         while ($row = mysqli_fetch_assoc($result)) { ?>
            <div class="col">
               <div class="card h-100">
                  <img src="./uploads/<?= htmlspecialchars($row['image']); ?>" height="500px" class="card-img-top" id="img" alt="<?= htmlspecialchars($row['name']); ?>" title="<?= htmlspecialchars($row['name']); ?>">
                  <div class="card-body align-items-end"> 
                     <h5 class="card-title"><?= htmlspecialchars($row['name']); ?></h5>
                     <h6>Quantity: <a class="card-title"><?= $row['quantity']; ?></a></h6>  
                     <h6 class="card-title"><?= htmlspecialchars($row['price']); ?>$</h6>
                     <form action="./cart.php?action=add&id=<?= $row['id']; ?>" method="POST">
                        <input type="number" name="quantity" min="1" max="<?= $row['quantity']; ?>" value="1" required>
                        <input type="hidden" name="fashion_name" value="<?= htmlspecialchars($row['name']); ?>">
                        <input type="hidden" name="price_per_each" value="<?= $row['price']; ?>">
                        <input type="hidden" name="img_source" value="<?= htmlspecialchars($row['image']); ?>">
